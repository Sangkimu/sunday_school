<?php
require_once __DIR__ . '/auth_guard.php';
require_once __DIR__ . '/db.php';

$message = $_SESSION['classes_message'] ?? ''; 
$messageType = $_SESSION['classes_message_type'] ?? 'success';
unset($_SESSION['classes_message'], $_SESSION['classes_message_type']);

$dbError = '';
$editClass = null;
$classes = [];
$teachers = [];
$basePath = rtrim(dirname($_SERVER['SCRIPT_NAME'] ?? $_SERVER['PHP_SELF'] ?? ''), '/');
$stats = [
    'total' => 0,
    'total_capacity' => 0,
    'avg_capacity' => 0,
];

try {
    $db = db_connect();

    $classes = db_fetch_all($db, 'SELECT class_id, class_name, level, teacher_name, capacity, schedule FROM classes ORDER BY class_name');
    $teachers = db_fetch_all($db, 'SELECT teacher_id, full_name FROM teachers WHERE status = "Active" ORDER BY full_name');

    $stats['total'] = count($classes);
    $stats['total_capacity'] = (int) db_scalar($db, 'SELECT COALESCE(SUM(capacity), 0) FROM classes');
    $stats['avg_capacity'] = $stats['total'] > 0 ? (int) ($stats['total_capacity'] / $stats['total']) : 0;

    if (isset($_GET['edit']) && ctype_digit((string) $_GET['edit'])) {
        $editClass = db_fetch_one($db, 'SELECT class_id, class_name, level, teacher_name, capacity, schedule FROM classes WHERE class_id = ?', [(int) $_GET['edit']]);
    }
} catch (Throwable $e) {
    $dbError = $e->getMessage();
}
?><!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Classes | AGC Bomet Area</title>
    <link rel="icon" type="image/jpeg" href="<?php echo htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8'); ?>/images/logo.jpg">
    <link rel="stylesheet" href="<?php echo htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8'); ?>/style.css?v=2">
</head>
<body>
    <div class="dashboard-container">
        <aside class="sidebar">
            <div class="brand">
                <img src="<?php echo htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8'); ?>/images/mu-logo.jpg" alt="AGC Bomet Area logo" class="brand-logo" onerror="this.onerror=null; this.src='<?php echo htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8'); ?>/images/logo.jpg';">
                <div>
                    <h1>AGC Bomet Area</h1>
                    <p>Sunday School Dashboard</p>
                </div>
            </div>

            <nav class="sidebar-nav">
                <p class="nav-label">Main</p>
                <a class="nav-item" href="<?php echo htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8'); ?>/dashboard.php">Dashboard</a>
                <a class="nav-item" href="<?php echo htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8'); ?>/children.php">Children</a>
                <a class="nav-item" href="<?php echo htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8'); ?>/teachers.php">Teachers</a>
                <a class="nav-item" href="<?php echo htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8'); ?>/attendance.php">Attendance</a>
                <a class="nav-item active" href="<?php echo htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8'); ?>/classes.php">Classes</a>
                <a class="nav-item" href="<?php echo htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8'); ?>/bible-stories.php">Bible Stories</a>
                <a class="nav-item" href="<?php echo htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8'); ?>/memory-verses.php">Memory Verses</a>
                <a class="nav-item" href="<?php echo htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8'); ?>/songs.php">Songs</a>
                <a class="nav-item" href="#">Plays</a>
                <a class="nav-item" href="<?php echo htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8'); ?>/calendar.php">Calendar of Events</a>
                <a class="nav-item" href="<?php echo htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8'); ?>/announcements.php">Announcements</a>
                <a class="nav-item" href="<?php echo htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8'); ?>/awards.php">Awards</a>
                <a class="nav-item" href="#">Reports</a>
                <a class="nav-item" href="#">Settings</a>
                <a class="nav-item" href="#">Profile</a>
                <a class="nav-item" href="<?php echo htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8'); ?>/logout.php">Log Out</a>
            </nav>
        </aside>

        <main class="main-content">
            <header class="top-bar">
                <div class="brand-block">
                    <img src="<?php echo htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8'); ?>/images/mu-logo.jpg" alt="AGC logo" class="top-brand-logo" onerror="this.onerror=null; this.src='<?php echo htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8'); ?>/images/logo.jpg';">
                    <div>
                        <p class="eyebrow">Sunday School</p>
                        <h2>Class Management</h2>
                    </div>
                </div>
                <div class="top-nav-links">
                    <a href="<?php echo htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8'); ?>/logout.php">Log Out</a>
                </div>
            </header>

            <section class="page-header">
                <div>
                    <p class="eyebrow">Classes</p>
                    <h3>Manage class groups</h3>
                    <p>Track class details, age groups, assigned teachers, and room locations.</p>
                </div>
                <a href="<?php echo htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8'); ?>/dashboard.php" class="text-link">Back to Dashboard</a>
            </section>

            <?php if ($dbError): ?>
                <section class="error-banner">
                    <p>Database error: <?php echo htmlspecialchars($dbError, ENT_QUOTES, 'UTF-8'); ?></p>
                </section>
            <?php endif; ?>

            <?php if ($message !== ''): ?>
                <section class="<?php echo $messageType === 'error' ? 'error-banner' : 'success-banner'; ?>">
                    <p><?php echo htmlspecialchars($message, ENT_QUOTES, 'UTF-8'); ?></p>
                </section>
            <?php endif; ?>

            <section class="summary-grid">
                <article class="summary-card">
                    <h4>Total Classes</h4>
                    <span><?php echo number_format($stats['total']); ?></span>
                </article>
                <article class="summary-card">
                    <h4>Total Capacity</h4>
                    <span><?php echo number_format($stats['total_capacity']); ?></span>
                </article>
                <article class="summary-card">
                    <h4>Avg per Class</h4>
                    <span><?php echo number_format($stats['avg_capacity']); ?></span>
                </article>
                <article class="summary-card">
                    <h4>Active Teachers</h4>
                    <span><?php echo number_format(count($teachers)); ?></span>
                </article>
            </section>

            <section class="table-card">
                <div class="table-header">
                    <h3><?php echo $editClass ? 'Edit Class' : 'Add New Class'; ?></h3>
                </div>
                <form action="<?php echo htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8'); ?>/classes_handler.php" method="post" class="panel-card">
                    <input type="hidden" name="action" value="<?php echo $editClass ? 'edit' : 'add'; ?>">
                    <?php if ($editClass): ?>
                        <input type="hidden" name="class_id" value="<?php echo (int) $editClass['class_id']; ?>">
                    <?php endif; ?>
                    <div class="children-grid">
                        <div>
                            <label for="class_name">Class Name</label>
                            <input type="text" id="class_name" name="class_name" value="<?php echo htmlspecialchars($editClass['class_name'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" required placeholder="e.g., Primary 2">
                        </div>
                        <div>
                            <label for="level">Age Group / Level</label>
                            <input type="text" id="level" name="level" value="<?php echo htmlspecialchars($editClass['level'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" placeholder="e.g., 7-8 years">
                        </div>
                        <div>
                            <label for="teacher_name">Teacher</label>
                            <input type="text" id="teacher_name" name="teacher_name" value="<?php echo htmlspecialchars($editClass['teacher_name'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" placeholder="Teacher name">
                        </div>
                        <div>
                            <label for="capacity">Capacity</label>
                            <input type="number" id="capacity" name="capacity" min="5" max="100" value="<?php echo htmlspecialchars((string) ($editClass['capacity'] ?? '30'), ENT_QUOTES, 'UTF-8'); ?>" required>
                        </div>
                        <div>
                            <label for="schedule">Schedule / Time</label>
                            <input type="text" id="schedule" name="schedule" value="<?php echo htmlspecialchars($editClass['schedule'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" placeholder="e.g., Sunday 10:00 AM">
                        </div>
                    </div>
                    <div style="margin-top: 1rem;">
                        <button type="submit" class="primary-btn"><?php echo $editClass ? 'Save Changes' : 'Add Class'; ?></button>
                        <?php if ($editClass): ?>
                            <a href="<?php echo htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8'); ?>/classes.php" class="text-link">Cancel</a>
                        <?php endif; ?>
                    </div>
                </form>
            </section>

            <section class="table-card">
                <div class="table-header">
                    <h3>Class Details</h3>
                </div>
                <div class="table-wrapper">
                    <table class="children-table">
                        <thead>
                            <tr>
                                <th>Class Name</th>
                                <th>Age Group</th>
                                <th>Teacher</th>
                                <th>Capacity</th>
                                <th>Schedule</th>
                                <th>Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php if (empty($classes)): ?>
                                <tr>
                                    <td colspan="6" style="text-align: center; padding: 2rem;">No classes yet. Add one to get started.</td>
                                </tr>
                            <?php else: ?>
                                <?php foreach ($classes as $class): ?>
                                    <tr>
                                        <td><?php echo htmlspecialchars($class['class_name'], ENT_QUOTES, 'UTF-8'); ?></td>
                                        <td><?php echo htmlspecialchars($class['level'] ?? '-', ENT_QUOTES, 'UTF-8'); ?></td>
                                        <td><?php echo htmlspecialchars($class['teacher_name'] ?? '-', ENT_QUOTES, 'UTF-8'); ?></td>
                                        <td><?php echo htmlspecialchars((string) $class['capacity'], ENT_QUOTES, 'UTF-8'); ?></td>
                                        <td><?php echo htmlspecialchars($class['schedule'] ?? '-', ENT_QUOTES, 'UTF-8'); ?></td>
                                        <td>
                                            <div style="display: flex; gap: 0.5rem;">
                                                <a href="<?php echo htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8'); ?>/classes.php?edit=<?php echo (int) $class['class_id']; ?>" class="text-link">Edit</a>
                                                <form action="<?php echo htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8'); ?>/classes_handler.php" method="post" style="display: inline;">
                                                    <input type="hidden" name="action" value="delete">
                                                    <input type="hidden" name="class_id" value="<?php echo (int) $class['class_id']; ?>">
                                                    <button type="submit" class="text-link" style="background: none; border: none; color: inherit; cursor: pointer; padding: 0;" onclick="return confirm('Are you sure?')">Delete</button>
                                                </form>
                                            </div>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
            </section>
        </main>
    </div>
</body>
</html>
